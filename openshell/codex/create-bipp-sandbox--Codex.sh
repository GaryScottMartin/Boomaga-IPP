...
PKGCONF_TMP="/tmp/bipp-pkgconf"
INSTALL_PKGCONF="install_pkgconf() { if [ -x '$PKGCONF_ROOT/usr/bin/pkgconf' ]; then return 0; fi; mkdir -p '$APT_STATE/lists/partial' '$APT_STATE/cache/archives/partial' '$PKGCONF_TMP' '$PKGCONF_ROOT' || return 1; cd '$PKGCONF_TMP' || return 1; apt-get -o APT::Sandbox::User=sandbox -o Dir::State::lists='$APT_STATE/lists' -o Dir::Cache='$APT_STATE/cache' update || return 1; apt-get -o APT::Sandbox::User=sandbox -o Dir::State::lists='$APT_STATE/lists' -o Dir::Cache='$APT_STATE/cache' download pkgconf-bin libpkgconf3 || return 1; for package in ./*.deb; do [ -f \"\$package\" ] || return 1; dpkg-deb --extract \"\$package\" '$PKGCONF_ROOT' || return 1; done; [ -x '$PKGCONF_ROOT/usr/bin/pkgconf' ]; }; if install_pkgconf; then export PATH='$PKGCONF_ROOT/usr/bin':\"\$PATH\"; export LD_LIBRARY_PATH='$PKGCONF_ROOT/usr/lib/x86_64-linux-gnu'\${LD_LIBRARY_PATH:+:\"\$LD_LIBRARY_PATH\"}; hash -r; echo \"pkg-config installed: \$(pkg-config --version)\"; else echo 'WARNING: pkg-config installation failed; continuing into Codex.' >&2; fi"
