...
if [ -n "${BIPP_VERIFY:-}" ]; then
  ...
  EXTRA=(--no-keep)
  ENTRY="set -e; $UPDATE_CODEX; $INSTALL_RUST; $CONFIGURE_SHELL; $CLONE; cd '$DIR'; echo \"PWD=\$(pwd)\"; test -d .git && echo GIT_OK; command -v codex >/dev/null && echo CODEX_OK; codex --version; command -v cargo >/dev/null && echo RUST_OK; rustc --version; cargo --version"
else
  NAME="${1:-BIPP-codex}"
  EXTRA=()
  ...
  ENTRY="set -e; $UPDATE_CODEX; $INSTALL_RUST; $CONFIGURE_SHELL; $CLONE; cd '$DIR'; if ! codex login status >/dev/null 2>&1; then echo 'Codex authentication required; complete the device-code flow in your browser.'; codex login --device-auth; fi; exec codex"
fi
