...
The Xilem migration’s Phases A through H are complete, host-verified, and merged
...
Focused verification passes with 10 layout-engine, 3 IPC, 1 backend, 3 config,
6 core, and 39 preview tests. Denali also verified real ET-3750 output: simplex
collate-on prints `123 123 123`, collate-off prints `111 222 333`, and duplex
preserves each document set in both modes. On 2026-07-26, a fresh Codex sandbox
completed `cargo check --workspace` with warnings and no errors, establishing the
workspace-wide compiler baseline and verifying the provisioned GLib, Cairo,
Poppler GLib, QPDF, and libclang dependency stack. On 2026-08-01,
`cargo test --workspace` passed all 62 tests with no failures; all doc-tests passed.
...
recorded in `docs/XILEM-vis-ver-results.txt`. Phase H is complete and Denali-verified:
deterministic booklet ordering/blank padding, selected-range-aware preview,
vector-preserving booklet and N-up assembly, short-edge duplex submission, and
mixed-orientation booklet/4-up output all match the preview.

Phase I is planned but must not be implemented until revised SRS/UIS documents
authorize its behavior. Its scope is complete `lpstat -e` CUPS destination
enumeration, self-queue recursion protection, a typed output-target model, and an
internal **Save to PDF** destination that reuses the imposed PDF pipeline.
...
- A fresh Codex sandbox completed `cargo check --workspace` with warnings and
  no errors on 2026-07-26. On 2026-08-01, `cargo test --workspace` passed all
  62 tests with no failures; all doc-tests passed.
...
- Phase F is complete and Denali-verified. Normal downstream submission forwards
  ...
  Phase H booklet and N-up submission assemble temporary vector-preserving imposed
  PDFs, submit them as one-up, and remove the artifacts through RAII cleanup.
